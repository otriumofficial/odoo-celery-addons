...
from odoo.addons.celery.models.celery_task import STATE_FAILURE, STATE_SUCCESS
import logging
...
class CeleryTask(models.Model):
    ...
    def write(self, vals):
        ...
        if 'state' in vals:
            ...
            for task in self:
                if task.state in [STATE_FAILURE, STATE_SUCCESS]:
        ...
        if celery_task_setting_id:
            ...
            if setting and setting.send_task_digest_email and setting.message_follower_ids:
                ...
                if failed_tasks:
                    ...
                    if template:
                        body_html = template.with_context(jammed_tasks=[], failed_tasks=failed_tasks)._render_template(template.body_html, 'celery.task.setting', setting.id)
                        mail_server = self.env['ir.mail_server']
                        message = mail_server.with_context(jammed_tasks=[], failed_tasks=failed_tasks).build_email(
                            email_from=self.sudo().env.user.email,
